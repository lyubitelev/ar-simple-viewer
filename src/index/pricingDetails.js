...
/**
 * Числа тарифов собраны здесь, чтобы внутри диалога цена не повторялась в нескольких
 * строках. Второй публичный носитель тех же чисел — статическая разметка карточек
 * в index.html: карточки не рендерятся из JS, поэтому при изменении
 * docs/product/pricing-policy.md нужно обновить оба места.
 */
const PRICES = {
    startSession: "0,30 ₽",
    proMonthly: "2 490 ₽",
    proIncluded: "10 000",
    businessMonthly: "9 990 ₽",
    businessIncluded: "50 000",
    overage: "0,20 ₽"
};

const TARIFF_DETAILS = {
    "Start": {
        price: `0 ₽ в месяц + ${PRICES.startSession} за оплачиваемую AR-сессию`,
        ...
        sections: [
            {
                ...
                items: [
                    ...
                    "Свои поддерживаемые и проверенные 3D-модели",
    ...
    "Pro": {
        price: `${PRICES.proMonthly} в месяц · ${PRICES.proIncluded} AR/XR-сессий включено`
            + ` · сверх лимита ${PRICES.overage} за сессию`,
    ...
    "Business": {
        price: `${PRICES.businessMonthly} в месяц · ${PRICES.businessIncluded} AR/XR-сессий включено`
            + ` · сверх лимита ${PRICES.overage} за сессию`,
